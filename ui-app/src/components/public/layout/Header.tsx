        ...
        <>
            ...
            <div className="lg:hidden mt-10 pb-10 flex items-center justify-around bg-base-100 shadow-sm border-b border-base-200 px-5">
                ...
                <div className="dropdown">
                    ...
                    <div tabIndex={0} role="button" className="btn btn-outline btn-primary border-2 border-base-200">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    >
                        {NAV_LINKS.map((link) =>
                            link.type === "download" ? (
                                <li key={link.href}>
                                    <a href={link.href} download className="text-lg hover:text-primary hover:bg-base-100 border-primary-1 transition-all duration-300">{link.label}</a>
                                </li>
                            ) : (
                                <li key={link.href}>
                                    <Link to={link.href} className="text-lg hover:text-primary hover:bg-base-100 border-primary-1 transition-all duration-300">{link.label}</Link>
                                </li>
                            )
                        )}
                        ...
                        <li>
                            <Link to={"#"} className="text-lg hover:text-primary hover:bg-base-100 border-primary-1 transition-all duration-300">
                                Langue (coming soon)
                            </Link>
                        </li>
                    </ul>
                ...
                <Link className="text-lg font-bold hover:text-primary  border-primary-1 transition-all duration-300 border-transparent" to={"/"}>RIDI OTOKO</Link>
            ...
            <div className="hidden lg:flex pt-5 pb-5 items-center justify-around bg-base-200 shadow-sm border-b border-base-200 px-10">
                ...
                <Link className="text-lg font-bold hover:text-primary hover:bg-base-200 border-primary-1 transition-all duration-300  border-transparent" to={"/"}>RIDI OTOKO</Link>

                <ul className="menu menu-horizontal px-1">
                    {NAV_LINKS.map((link) =>
                        link.type === "download" ? (
                            <li key={link.href}>
                                <a href={link.href} download className="hover:text-primary hover:bg-base-200 border-primary-1 transition-all duration-300  border-b-2 border-transparent hover:border-primary hover:rounded-none rounded-none">{link.label}</a>
                            </li>
                        ) : (
                            <li key={link.href}>
